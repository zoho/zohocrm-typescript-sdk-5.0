...
export class JSONConverter extends Converter {
		...
		for (let keyName of Array.from(keyModified.keys())) {
			...
			if (keyValue != null) {
				if (Object.keys(keyDetail).length > 0) {
					...
					{
						let key = await this.keyINJSONDetails(keyValue.constructor.name);

						if (keyValue instanceof Date)
						{
							if (keyValue.getHours() == 0 && keyValue.getMinutes() == 0 && keyValue.getSeconds() == 0)
							{
								jsonValue = DatatypeConverter.postConvert(keyValue, "Date");
							}
							else
							{
								jsonValue = DatatypeConverter.postConvert(keyValue, "DateTime");
							}
						}
					...
					else
					{
						if (customHandling && keyValue.constructor.name == Constants.ARRAY_KEY) {
							let key1 = await this.keyINJSONDetails(keyValue[0].constructor.name);
							if (key1 != null) {
								...
								jsonValue = await this.redirectorForObjectToJSON(keyValue).catch((err) => {
									throw err;
								});
							}
						} else {
	...
	private async getModuleDetailFromUserSpecJSON(module: string) {
		var recordFieldDetailsPath = join(await Converter.getEncodedFileName());

		var moduleDetailFromUserSpecJSON = Initializer.getJSON(recordFieldDetailsPath)

		var moduleDetail = await Utility.getJSONObject(moduleDetailFromUserSpecJSON, module)
