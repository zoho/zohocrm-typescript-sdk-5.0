...
class CreatePortals
{
    static async createPortals()
    {
        ...
        let portal:  ZOHOCRMSDK.Portals.Portals = new ZOHOCRMSDK.Portals.Portals();
        portal.setName("PortalAPITest100");
        portals.push(portal);
        bodyWrapper.setPortals(portals);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Portals.ActionHandler.MasterModel> = await portalsOperations.createPortal(bodyWrapper);
        if (response != null)
        {
...
CreatePortals.initializeAndCall();
