...
{
    static async createPurchaseOrders(module: string)
    {
        ...
        let vendor : ZOHOCRMSDK.Record.Record = new ZOHOCRMSDK.Record.Record();
        await vendor.addFieldValue(ZOHOCRMSDK.Record.Field.Vendors.ID, BigInt("44028001392120"));
        ...
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Record.ActionHandler.MasterModel>= await recordOperations.createRecords(module, request, headerInstance);
        if (response != null)
        {
...
CreatePurchaseOrders.initializeAndCall();
