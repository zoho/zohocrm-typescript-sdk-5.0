...
class CreateEvents
{
    static async createEvents(moduleAPIName: string)
    {
        ...
        await record1.addFieldValue(ZOHOCRMSDK.Record.Field.Events.START_DATETIME, new Date(2023,10,9,11,32,33));
        let participants : ZOHOCRMSDK.Record.Participants[] = [];
        let participant1 : ZOHOCRMSDK.Record.Participants = new ZOHOCRMSDK.Record.Participants();
        participant1.setEmail('abc@gmail.com');
        participant1.setType('email');
        participant1.setId(BigInt("440280254001"));
        participants.push(participant1);
        let participant2 : ZOHOCRMSDK.Record.Participants = new ZOHOCRMSDK.Record.Participants();
        participant2.addKeyValue('participant', BigInt("4003423423434"));
        ...
        await record1.addFieldValue(ZOHOCRMSDK.Record.Field.Events.END_DATETIME, new Date(2023, 10, 10, 11, 30, 22));
        let reminderList : ZOHOCRMSDK.Record.Reminder[]  = [];
        let remindAt : ZOHOCRMSDK.Record.Reminder = new ZOHOCRMSDK.Record.Reminder();
        remindAt.setUnit(20);
        remindAt.setPeriod("Minutes");
        reminderList.push(remindAt);
        let remindAt1 : ZOHOCRMSDK.Record.Reminder = new ZOHOCRMSDK.Record.Reminder();
        remindAt1.setPeriod("minutes");
        remindAt1.setUnit(15);
        reminderList.push(remindAt1);
        record1.addFieldValue(ZOHOCRMSDK.Record.Field.Events.REMIND_AT, reminderList);
        record1.addKeyValue('$se_module', 'leads');
        let whatId : ZOHOCRMSDK.Record.Record = new ZOHOCRMSDK.Record.Record();
        whatId.setId(BigInt("4300234243"));
    ...
    static async initializeAndCall() {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await CreateEvents.createEvents("Events");
    }
}
CreateEvents.initializeAndCall();
