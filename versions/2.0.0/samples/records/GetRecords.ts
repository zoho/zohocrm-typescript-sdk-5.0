...
class GetRecords{
    ...
    static async getRecords(moduleAPIName: string) {
        ...
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.CONVERTED, "both");
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.CVID, "34770610087501");
        let ids = ["External121123456", "34770614352001n"];
        for (let id of ids) {
            await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.IDS, id);
        }
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.UID, "34770615181008");
        let fieldNames = ["Company", "Email"];
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.FIELDS, fieldNames.toString());
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.SORT_BY, "Email");
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.SORT_ORDER, "desc");
        ...
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.TERRITORY_ID, "34096430505351");
        await paramInstance.add(ZOHOCRMSDK.Record.GetRecordsParam.INCLUDE_CHILD, "true");
