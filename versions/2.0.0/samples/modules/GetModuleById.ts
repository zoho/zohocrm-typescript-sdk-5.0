...
export class GetModuleById {
     ...
     */
    static async getModuleById(id : bigint) {
        let modulesOperations : ZOHOCRMSDK.Modules.ModulesOperations= new ZOHOCRMSDK.Modules.ModulesOperations();
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Modules.ResponseHandler.MasterModel>= await modulesOperations.getModule(id);
        if (response != null) {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetModuleById.getModuleById(BigInt("440280037"));    }
}
GetModuleById.initializeAndCall();
