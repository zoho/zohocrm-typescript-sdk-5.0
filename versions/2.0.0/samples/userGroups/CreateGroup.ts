...
class CreateGroup
{
    static async createGroup()
    {
        ...
        source1.setId(BigInt("440280254001"));
        await source.setSource(source1);
        sources.push(source);
        user1.setSources(sources);
        userList.push(user1);
        request.setUserGroups(userList);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.GroupsUser.ActionHandler.MasterModel>= await userGroupOperations.createGroups(request);
        if (response != null)
        {
