...
export class UpdateTags
{
    public static async updateTags(moduleAPIName: string) {
        ...
        //Call updateTags method that takes ZOHOCRMSDK.Tags.BodyWrapper instance and ZOHOCRMSDK.ParameterMap instance as parameter
        let response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Tags.ActionHandler.MasterModel> = await tagsOperations.updateTags(request, paramInstance);
        if (response != null) {
    ...
    static async initializeAndCall() {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await UpdateTags.updateTags("Leads");
    }
}
