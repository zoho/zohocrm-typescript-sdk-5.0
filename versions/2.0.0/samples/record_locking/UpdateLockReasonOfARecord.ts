...
class UpdateLockReasonOfARecord
{
    public static async updateLockReasonOfARecord(moduleAPIName : string, recordId : bigint, lockId: bigint)
    {
        ...
        lockRecord.setLockedReasonS("deal in queue");
        data.push(lockRecord);
        request.setData(data);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.LockingRecord.ActionHandler.MasterModel> = await recordLockingOperations.updateLockReasonOfARecord(lockId, recordId, moduleAPIName, request);
        if (response != null)
        {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await UpdateLockReasonOfARecord.updateLockReasonOfARecord("Leads", BigInt("44028001787377"), BigInt("44028001821006"));
    }
}
UpdateLockReasonOfARecord.initializeAndCall();
