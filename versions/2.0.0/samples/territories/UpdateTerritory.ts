...
{
    static async updateTerritory(id: bigint)
    {
        ...
        manager.setId(BigInt("4402801394119"));
        await territory.setManager(manager);
        territories.push(territory);
        request.setTerritories(territories);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Territories.ActionHandler.MasterModel> = await territoriesOperations.updateTerritory(id, request);
        if (response != null) {
    ...
    static async initializeAndCall() {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await UpdateTerritory.updateTerritory(BigInt("334234"));
    }
}
UpdateTerritory.initializeAndCall();
