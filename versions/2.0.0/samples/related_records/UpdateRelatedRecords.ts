...
export class UpdateRelatedRecords
{
    public static async updateRelatedRecords(moduleAPIName: string, recordId: bigint, relatedListAPIName: string) {
         ...
         * 2 -> Value
         */
        record1.setId(BigInt("34770013245001"));
         ...
         * 2 -> Value
         */
        record2.addKeyValue("id", BigInt("34770613236002"));
        ...
        let response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.RecordsRelated.ActionHandler.MasterModel> = await relatedRecordsOperations.updateRelatedRecords(recordId, request);
        if (response != null) {
    ...
    static async initializeAndCall()
    {
        ...
        let recordId : bigint= BigInt("3409643798007");
        let relatedModuleAPIName  : string= "Price_Books";
        await UpdateRelatedRecords.updateRelatedRecords(moduleAPIName, recordId, relatedModuleAPIName);
    }
}
UpdateRelatedRecords.initializeAndCall()
