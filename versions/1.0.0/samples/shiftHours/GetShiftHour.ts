...
class GetShiftHour
{
    static async getShiftHour(id: bigint)
    {
        let shiftHoursOperations : ZOHOCRMSDK.HoursShift.ShiftHoursOperations = new ZOHOCRMSDK.HoursShift.ShiftHoursOperations("");
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.HoursShift.ResponseHandler.MasterModel>=await shiftHoursOperations.getShiftHour(id);
        if (response != null)
        {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetShiftHour.getShiftHour(BigInt("44028001729028"));
    }
}
GetShiftHour.initializeAndCall();
