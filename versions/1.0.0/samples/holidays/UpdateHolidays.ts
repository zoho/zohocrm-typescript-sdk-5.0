...
class UpdateHolidays
{
    ...
    {
        let holidaysOperations :ZOHOCRMSDK.Holidays.HolidaysOperations= new ZOHOCRMSDK.Holidays.HolidaysOperations("343423412313233");
        ...
        holiday.setId(BigInt("4023423421432423"));
        ...
        shifthour.setId(BigInt("44032232432423"));
        await holiday.setShiftHour(shifthour);
        holiday.setYear(2020);
        holidays.push(holiday);
        request.setHolidays(holidays);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Holidays.ActionHandler.MasterModel> = await holidaysOperations.updateHolidays(request);
        if (response != null)
        {
...
UpdateHolidays.initializeAndCall();
