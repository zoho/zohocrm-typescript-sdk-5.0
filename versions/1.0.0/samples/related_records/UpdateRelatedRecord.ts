...
export class UpdateRelatedRecord
{
    public static async updateRelatedRecord(moduleAPIName: string, recordId: bigint, relatedListAPIName: string, relatedListId: bigint) {
        ...
        record1.addFieldValue(ZOHOCRMSDK.Record.Field.Notes.NOTE_CONTENT, "Updated");
        ...
        let response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.RecordsRelated.ActionHandler.MasterModel> = await relatedRecordsOperations.updateRelatedRecord(relatedListId, recordId, request);
        if (response != null) {
    ...
    static async initializeAndCall()
    {
        ...
        let relatedId : bigint= BigInt("44028001728003");
        await UpdateRelatedRecord.updateRelatedRecord(moduleAPIName, recordId, relatedModuleAPIName, relatedId);
    }
}
