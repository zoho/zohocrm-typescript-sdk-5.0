...
export class GetLayouts
{
    public static async getLayouts(moduleAPIName: string) {
        ...
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Layouts.ResponseHandler.MasterModel>= await layoutsOperations.getLayouts(paramInstance);
        if (response != null) {
    ...
    static async initializeAndCall() {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetLayouts.getLayouts("Leads");
    }
}
