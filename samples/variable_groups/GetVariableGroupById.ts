...
export class GetVariableGroupById{
    public static async getVariableGroupById(variableGroupId: string) {
        //example
        ...
        let response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.GroupsVariable.ResponseHandler.MasterModel> = await variableGroupsOperations.getVariableGroupById(variableGroupId);
        if (response != null) {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetVariableGroupById.getVariableGroupById("440280345005");
    }
}
