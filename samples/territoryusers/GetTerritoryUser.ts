...
{
    static async getTerritoryUser(user: bigint, territory: bigint)
    {
        ...
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.TerritoryUsers.ResponseHandler.MasterModel>= await territoryUsersOperations.getTerritoryUser(user, territory);
        if (response != null) {
    ...
    static async initializeAndCall() {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetTerritoryUser.getTerritoryUser(BigInt("440280254001"), BigInt("44028001305227"));
    }
}
GetTerritoryUser.initializeAndCall();
