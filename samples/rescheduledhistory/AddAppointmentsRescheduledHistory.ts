...
class AddAppointmentsRescheduledHistory
{
    static async addAppointmentRescheduledHistory()
    {
        ...
        await rescheduleHistory.setRescheduledFrom(new Date('September 22, 2023 12:00:00'));
        await rescheduleHistory.setRescheduledTo(new Date('September 21, 2023 06:35:32'));
        ...
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.HistoryReschedule.ActionHandler.MasterModel>= await rescheduleHistoryOperations.addAppointmentsRescheduledHistory(request);
        if (response != null) {
...
AddAppointmentsRescheduledHistory.initializeAndCall();
