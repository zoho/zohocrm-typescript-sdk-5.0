...
class UpdateGroups
{
    static async updateGroups()
    {
        ...
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.GroupsUser.ActionHandler.MasterModel>= await userGroupOperations.updateGroups(request);
        if (response != null)
        {
...
UpdateGroups.initializeAndCall();
