...
export class GetRelatedEmail
{
    static async getRelatedEmail(moduleAPIName: string, id: bigint, messageId: string)
    {
        ...
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.EmailRecordsRelated.ResponseHandler.MasterModel>= await emailRelatedOperations.getEmailsRelatedRecord(messageId);
        if (response != null)
        {
            ...
            if (responseHandler instanceof ZOHOCRMSDK.EmailRecordsRelated.ResponseWrapper)
            {
                ...
                if (emailTemplates != null)
                {
                    emailTemplates.forEach(emailTemplate => {
                        ...
                        let attechments :  ZOHOCRMSDK.EmailRecordsRelated.Attachments[] = emailTemplate.getAttachments();
                        if (attechments != null) {
    ...
    static async initializeAndCall() {
        ...
        let messageId :string= "c6085fae06cbd7b75001d80fab46bcd790ed59cd3793ccd16fcea97f201b3";
        await GetRelatedEmail.getRelatedEmail(moduleAPIName, id, messageId);
    }
}
GetRelatedEmail.initializeAndCall();
