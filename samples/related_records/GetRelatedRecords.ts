...
export class GetRelatedRecords
{
    public static async getRelatedRecords(moduleAPIName: string, recordId: bigint, relatedListAPIName: string) {
        ...
        let response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.RecordsRelated.ResponseHandler.MasterModel> = await relatedRecordsOperations.getRelatedRecords(recordId, paramInstance, headerInstance);
        if (response != null) {
...
GetRelatedRecords.initializeAndCall();
