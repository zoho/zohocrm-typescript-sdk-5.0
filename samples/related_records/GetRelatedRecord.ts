...
export class GetRelatedRecord
{
    public static async getRelatedRecord(moduleAPIName: string, recordId: bigint, relatedListAPIName: string, relatedListId: bigint, destinationFolder: string) {
        ...
        let response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.RecordsRelated.ResponseHandler.MasterModel> = await relatedRecordsOperations.getRelatedRecord(relatedListId, recordId, paramInstance , headerInstance);
        if (response != null) {
    ...
    static async initializeAndCall()
    {
        ...
        let relatedListId : bigint= BigInt("44028001627014");
        let destinationFolder : string= "/Users/user/Desktop";
        await GetRelatedRecord.getRelatedRecord(moduleAPIName, recordId, relatedModuleAPIName, relatedListId, destinationFolder);
    }
}
GetRelatedRecord.initializeAndCall();
