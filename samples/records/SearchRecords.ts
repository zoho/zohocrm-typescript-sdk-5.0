...
class SearchRecords{
     ...
     */
    static async searchRecords(moduleAPIName: string) {
        //example
        ...
        await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.EMAIL, "abc@zgmail.com");
        ...
        await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.CVID, "44028029342")
        ...
        await paramInstance.add(ZOHOCRMSDK.Record.SearchRecordsParam.CRITERIA, "(External:in:TestExternal123)");
        let headerInstance : ZOHOCRMSDK.HeaderMap = new ZOHOCRMSDK.HeaderMap();
        headerInstance.add(ZOHOCRMSDK.Record.SearchRecordsHeader.X_EXTERNAL, "Leads.External");
        //Call searchRecords method that takes moduleAPIName, ParameterMap Instance and headerInstance as parameter
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Record.ResponseHandler.MasterModel>= await recordOperations.searchRecords(moduleAPIName, paramInstance, headerInstance);
        if (response != null) {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await SearchRecords.searchRecords("Leads");
    }
}
SearchRecords.initializeAndCall();