...
export class CreateTerritories
{
    static async createTerritories()
    {
        ...
        reportingTo.setId(BigInt("44028001305227"));
        await territory.setReportingTo(reportingTo);
        let manager = new ZOHOCRMSDK.Territories.Manager();
        manager.setId(BigInt("440280254001"));
        await territory.setManager(manager);
        territories.push(territory);
        request.setTerritories(territories);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Territories.ActionHandler.MasterModel> = await territoriesOperations.createTerritories(request);
        if (response != null) {
...
CreateTerritories.initializeAndCall();
