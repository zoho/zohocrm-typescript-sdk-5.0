...
export class TransferAndDeleteTerritories
{
    static async transferAndDeleteTerritories()
    {
        ...
        territory.setId(BigInt("44028001647146"));
        territory.setTransferToId(BigInt("44028001305227"));
        territory.setDeletePreviousForecasts(false);
        territories.push(territory);
        request.setTerritories(territories);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.Territories.ActionHandler.MasterModel>= await territoriesOperations.transferAndDeleteTerritories(request);
        if (response != null) {
...
TransferAndDeleteTerritories.initializeAndCall();
