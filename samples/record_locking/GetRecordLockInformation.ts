...
{
    public static async getRecordLockInformation(moduleAPIName: string, recordId: bigint)
    {
        ...
        await paramInstance.add(ZOHOCRMSDK.LockingRecord.getrecordlockinformationParam.FIELDS, "Record_Locking_Rule_Id__s");
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.LockingRecord.ResponseHandler.MasterModel> = await recordLockingOperations.getRecordLockInformation(recordId, moduleAPIName, paramInstance);
        if (response != null)
        {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetRecordLockInformation.getRecordLockInformation("Leads", BigInt("44028001787377"));
    }
}
GetRecordLockInformation.initializeAndCall();
