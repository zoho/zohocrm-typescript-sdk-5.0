...
{
    static async validateBeforeTransfer(userId: bigint, territoryId: bigint)
    {
        ...
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.TerritoriesUsers.ValidationHandler.MasterModel>= await usersTerritoriesOperations.validateBeforeTransfer(territoryId, userId);
        if (response != null)
        {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await ValidateBeforeTransfer.validateBeforeTransfer(BigInt('44028001647189'), BigInt("44028001647202"));
    }
}
ValidateBeforeTransfer.initializeAndCall();
