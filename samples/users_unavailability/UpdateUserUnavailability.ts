...
{
    static async updateUserUnavailability(id: string)
    {
        ...
        user.setId(BigInt("4402800254001"));
        await user1.setUser(user);
        userList.push(user1);
        request.setUsersUnavailability(userList);
        let response : ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.UnavailabilityUsers.ActionHandler.MasterModel>= await usersOperations.updateUserUnavailability(id, request);
        if(response != null)
        {
    ...
    static async initializeAndCall()
    {
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await UpdateUserUnavailability.updateUserUnavailability("32323232");
    }
}
UpdateUserUnavailability.initializeAndCall();
