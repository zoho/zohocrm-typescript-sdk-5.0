...
export class GetContactRole
{
    public static async getContactRole(contactRoleId: bigint) {
        ...
        let response: ZOHOCRMSDK.APIResponse<ZOHOCRMSDK.ContactRoles.ResponseHandler.MasterModel> = await contactRolesOperations.getRole(contactRoleId);
        if (response !== null) {
    ...
    static async initializeAndCall(){
        ...
        await (await new ZOHOCRMSDK.InitializeBuilder())
            ...
            .token(token)
            .initialize();
        await GetContactRole.getContactRole(BigInt("44028001632005"));
    }
}
